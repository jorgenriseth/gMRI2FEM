...
def diffusion_model(
    ...
    timevec, data = read_concentration_data(inputfile, "concentration_domain")
    ...
    with df.HDF5File(df.MPI.comm_world, str(inputfile), "r") as f:
        if "source" in f.attributes("concentration_domain"):
            f_code = f.attributes("concentration_domain")["source"]
            f_coeffs = json.loads(f.attributes("concentration_domain")["coeffs"])
